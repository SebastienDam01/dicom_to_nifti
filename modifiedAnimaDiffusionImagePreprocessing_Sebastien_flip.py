...
if (args.no_disto_correction is False or args.no_brain_masking is False) and not args.t1 == "":
	...
	brainExtractionCommand = ["python", animaScriptsDir + "brain_extraction/animaAtlasBasedBrainExtraction.py", "-i", args.t1, "-m", T1Prefix + "_brainMask.nii.gz", "-b", T1Prefix + "_masked.nii.gz"]
...
if args.no_disto_correction is False:
    if not (args.reverse == ""):
        ...
        correctionCommand = [animaDir + "animaPyramidalBMRegistration", "-r", tmpDWIImagePrefix + "_B0.nrrd", \
                     "-m", T1Prefix + "_masked.nii.gz", "-o", T1Prefix + "_dwi.nrrd","-O", T1Prefix + "_rig_tr.txt"]
...
if args.no_brain_masking is False:
    ...
    if args.t1 == "":
        shutil.move(tmpDWIImagePrefix + "_forBrainExtract_brainMask.nrrd", dwiImagePrefix + "_brainMask.nii.gz")
    else:
        T1Prefix = os.path.splitext(args.t1)[0]
        if os.path.splitext(args.t1)[1] == '.gz':
            T1Prefix = os.path.splitext(T1Prefix)[0]
        ...
        t1RegistrationCommand = [animaDir + "animaPyramidalBMRegistration", "-r",
                                 tmpDWIImagePrefix + "_forBrainExtract.nrrd", "-m", T1Prefix + "_masked.nii.gz", "-o",
        ...
        command = [animaDir + "animaApplyTransformSerie", "-i", T1Prefix + "_brainMask.nii.gz", "-t",
                                                T1Prefix + "_rig_tr.xml", "-o", dwiImagePrefix + "_brainMask.nii.gz", "-g",
                                                tmpDWIImagePrefix + "_forBrainExtract.nrrd", "-n", "nearest"]
        call(command)
        brainExtractionCommand = [animaDir + "animaMaskImage", "-i", outputImage, "-m", dwiImagePrefix + "_brainMask.nii.gz", \
                                                                            "-o", tmpDWIImagePrefix + "_masked.nii.gz"]
        call(brainExtractionCommand)
        outputImage = tmpDWIImagePrefix + "_masked.nii.gz"
...
if args.no_brain_masking is False:
    dtiEstimationCommand += ["-m", dwiImagePrefix + "_brainMask.nii.gz"]
...
DTITractography = [animaDir + "animaDTITractography", "--max-length", "300" ,"--min-length", "10","--nb-fibers", "2",\
                   "-o", dwiImagePrefix + "_fiber_Tensor.vtk","-s", dwiImagePrefix + "_brainMask.nii.gz","-i", dwiImagePrefix + "_Tensors.nii.gz"]
...
'''
print(".............Register T13D sur MNI")

t1RegistrationCommand = [animaDir + "animaPyramidalBMRegistration","-r",'/srv/tempdd/jcoloign/Template/MNI152_T1_1mm_masked.nii.gz',"-m",T1Prefix + "_rig.nrrd","-o",T1Prefix + "_masked_rig_MNI.nii.gz","-O",T1Prefix + "_masked_rig_MNI.txt","-p","3","-l","0","--sp","2","--ot","0"]
call(t1RegistrationCommand)

#t1RegistrationCommand = [animaDir + "animaPyramidalBMRegistration","-r",'/srv/tempdd/jcoloign/Template/MNI152_T1_1mm_brain.nii.gz',"-m",T1Prefix + "_masked_rig_MNI.nii.gz","-o",T1Prefix + "_masked_aff_MNI.nii.gz","-O",T1Prefix + "_masked_aff_MNI.txt","-p","3","-l","0","--sp","2","--ot","2"]
#call(t1RegistrationCommand)

t1RegistrationNonRigCommand = [animaDir + "animaDenseSVFBMRegistration","-r",'/srv/tempdd/jcoloign/Template/MNI152_T1_1mm_masked.nii.gz',"-m",T1Prefix + "_masked_rig_MNI.nii.gz","-o",T1Prefix + "_masked_nonrig_MNI.nii.gz","-O",T1Prefix + "_masked_transf_MNI.nii.gz","-p","3","-l","0"]
call(t1RegistrationNonRigCommand)

command = [animaDir + "animaTransformSerieXmlGenerator","-i",T1Prefix + "_masked_rig_MNI.txt","-i",T1Prefix + "_masked_transf_MNI.nii.gz","-o",T1Prefix + "_masked_transf_MNI.xml"]
call(command)

command = [animaDir + "animaTransformSerieXmlGenerator","-i", T1Prefix + "_rig_tr.txt", "-i",T1Prefix + "_masked_rig_MNI.txt","-i",T1Prefix + "_masked_transf_MNI.nii.gz","-o",dwiImagePrefix + "_transf_MNI.xml"]
call(command)

print(".............Registration T1 into template")

command = [animaDir + "animaApplyTransformSerie","-i",dwiImagePrefix + "_FA.nii.gz","-t",dwiImagePrefix + "_transf_MNI.xml","-o",dwiImagePrefix + "_FA_MNI.nii.gz","-g",'/srv/tempdd/jcoloign/Template/MNI152_T1_1mm_masked.nii.gz']
call(command)
command = [animaDir + "animaApplyTransformSerie","-i",dwiImagePrefix + "_AD.nii.gz","-t",dwiImagePrefix + "_transf_MNI.xml","-o",dwiImagePrefix + "_AD_MNI.nii.gz","-g",'/srv/tempdd/jcoloign/Template/MNI152_T1_1mm_masked.nii.gz']
call(command)
command = [animaDir + "animaApplyTransformSerie","-i",dwiImagePrefix + "_ADC.nii.gz","-t",dwiImagePrefix + "_transf_MNI.xml","-o",dwiImagePrefix + "_ADC_MNI.nii.gz","-g",'/srv/tempdd/jcoloign/Template/MNI152_T1_1mm_masked.nii.gz']
call(command)
command = [animaDir + "animaApplyTransformSerie","-i",dwiImagePrefix + "_RD.nii.gz","-t",dwiImagePrefix + "_transf_MNI.xml","-o",dwiImagePrefix + "_RD_MNI.nii.gz","-g",'/srv/tempdd/jcoloign/Template/MNI152_T1_1mm_masked.nii.gz']
call(command)


command = [animaDir + "animaTransformSerieXmlGenerator","-i", T1Prefix + "_masked_rig_MNI.txt", "-o",T1Prefix + "_transf_CACP.xml"]
call(command)

command = [animaDir + "animaApplyTransformSerie","-i",outputImage,"-t",T1Prefix + "_transf_CACP.xml","-o",dwiImagePrefix + "_preprocessed_CACP.nii.gz","-g",'/temp_dd/igrida-fs1/jcoloign/longidep/MNI152_T1_1mm_masked.nii.gz']
call(command)
